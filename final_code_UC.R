...
## "Predicting Corticosteroid Free Endoscopic Remission with Vedolizumab in Ulcerative Colitis" (2017)
...
for(m in 1:50){
  ...
  var_cs = c(names(x_cs)[-1],"CALPRO_DIFF","PC_DIFF") #CALPRO_DIFF: slope of FCP, PC_DIFF: slope of VDZ level
...
var_cs = c(names(x_cs)[-1],"CALPRO_DIFF","PC_DIFF")
...
###############################################################################
# Simpler model: baseline FCP
sbsl_roc = roc(y[,"OUTCOME"],x_bsl[,"CALPRO_Week0"],ci=T)
sbsl_roc #ROC

sbsl_roc$direction
co_sbsl = coords(sbsl_roc,"b", best.method="closest.topleft",ret=c("thre","spec","sens","ppv","1-npv","tp","tn","fp","fn"))
co_sbsl #get the best cutoff and other quantities

##############################################################################
# Simpler model: FCP/VDZ at week 6
x = rep(NA,nrow(x_lgt))
for(i in 1:length(x)){
  if(x_lgt$PC_Week6[i]!=0){
    x[i]=x_lgt$CALPRO_Week6[i]/x_lgt$PC_Week6[i]
  }
}
tmp = quantile(na.omit(x),prob=0.99)
x[is.na(x)]=tmp
...
##############################################################################
# Simpler model: FCP at week 6
x = x_lgt$CALPRO_Week6
